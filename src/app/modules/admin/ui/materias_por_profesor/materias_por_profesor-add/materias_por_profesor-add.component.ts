...
@Component({
  selector: 'materias_por_profesor-add',
  templateUrl: './materias_por_profesor-add.component.html',
  styleUrls: ['./materias_por_profesor.component.scss'],
  standalone: true,
...
export class Formsmaterias_por_profesor implements OnInit {
  ...
  obtenerProfesores() {
    this.http.get<Profesores[]>('http://localhost:8080/api/profesores').subscribe(
      profesores => {
  ...
  registrar() {
    if (this.form.valid) {
      ...
      this.http.post('http://localhost:8080/api/materiaprofesor', data)
        .subscribe(
          response => {
